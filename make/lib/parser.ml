...
let not_hash = function '#' -> false | _ -> true
let not_close_bracket = function ')' -> false | _ -> true
let not_dollar = function '$' -> false | _ -> true
let not_backslash = function '\\' -> false | _ -> true
let not_colon = function ':' -> false | _ -> true

let all_pred preds el = List.for_all (fun f -> f el) preds
let some_pred preds el = List.exists (fun f -> f el) preds

let multy_string = 
  many(take_while (all_pred [not_newline; not_backslash; not_colon; not_hash]) <* string "\\n")
...
(* Comment *)
let comment_string = 
  let multy_string = string "\\n" *> take_while (all_pred [not_backslash; not_newline; not_hash]) in
  let start_string = char '#' *> take_while1 (all_pred [not_backslash; not_newline; not_hash]) in
  let compare _ _ _ = [()] in
  lift3 compare start_string (many multy_string) (many1 end_of_line)

let empty_string = 
  (take_while1 is_empty_char *> many1 end_of_line) <|> many1 end_of_line
...
let words_declaration =
  many (skip_while (some_pred [is_empty_char]) *> 
        take_while1 (all_pred [not_backslash; not_colon; not_hash; (Fun.negate is_empty_char)])) 
...
let substrings_declaration = 
  ...
  let var_dec = 
    peek_char_fail >>= function
      ...
      | 'a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '_' | '@' | '^' | '$' -> take 1 >>| var_constructor
      | c -> raise (InvalidVarName (Char.escaped c))
  in
  let var_declaration = char '$' *> var_dec in
  let str_declaration = take_while1 not_dollar in
  (lift List.concat 
    (many (lift3 concat_list_str_list (many var_declaration) str_declaration (many var_declaration))))
...
let exprs =
  let rule_constructor t p c = 
    RULE({ targets=t; prerequisites=p; commands=c})
  in
  let rule_declaration = 
    (lift3 rule_constructor 
      targets_declaration 
      prerequisites_declaration 
      commands_declaration) <* many end_of_line
  in
